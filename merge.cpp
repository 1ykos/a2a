...
double inline universal_distribution(const double x)
{
  ...
  //return (1-1.0/16)*0.5*exp(-abs(x)/1900)/1900
  //  +wmath::universal_distribution(floor(abs(x)))/16;
  const double a=1.0/170;
...
struct cell_proposer{
  ...
  const inline matrix<double,9,1> operator()(matrix<double,9,1> x) {
    ...
    R(2,1) = x(7);
    R(2,2) = x(8);
    const double epsilon = 1e-7; // 3e-7
    ...
    transform(
        ...
        )
        {
          ...
          if constexpr (true) {
            ...
            matrix<double,4,1> best_x = x;
            for (int i=-4;i<=4;++i) {
              matrix<double,4,1> _x = x;
              _x(0)=x(0)*pow(2,i);
              const double value = get<0>(target(_x));
              if (value<best_value) {
                best_x = _x;
                best_value = value;
              }
